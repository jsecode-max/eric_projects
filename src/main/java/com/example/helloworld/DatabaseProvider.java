...
public class DatabaseProvider extends ContentProvider {
    ...
    private MyDatabaseHelper dbHelper;  //内容提供器将基于这个数据库提供数据
    ...
    public boolean onCreate() {
        // 初始化内容提供器的数据，这里初始化数据库
        dbHelper = new MyDatabaseHelper(getContext(), "BookStoreNew.db", null, 11);
        return true;
    }
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        SQLiteDatabase db = dbHelper.getReadableDatabase(); //初始化数据库，建库、建表
        Cursor cursor = null;
        switch (uriMatcher.match(uri)) {
            case BOOK_DIR:
                ...
                break;
            case BOOK_ITEM:
                String bookId = uri.getPathSegments().get(1);   //获取path的具体行id
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (uriMatcher.match(uri)) {
            case BOOK_DIR:
            case BOOK_ITEM:
                long newBoodId = db.insert("Book", null, values);
                uriReturn = Uri.parse("content://" + AUTHORITY + "/book/" + newBoodId);
                break;
            case CATEGORY_DIR:
            case CATEGORY_ITEM:
